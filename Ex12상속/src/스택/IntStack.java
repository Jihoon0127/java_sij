...
public class IntStack {
	...
	public int push(int x) { //원하는 값(x)를 스택에 삽입, 0번 인덱스부터 차근차근 채우기
		stk[ptr++] = x; // 포인터가 가리키는 위치에 값을 삽입한 후 포인터를 1증가
						// ptr++ : 데이터 개수 카운팅 , 그다음 데이터는 그다음칸에 들어갈수 있도록
		return x;
	}
	...
	//특정 값 몇 번 인덱스 있는지 찾기 : TOP(위에서부터)!!
	public int indexOf(int x) { //x: 찾고 싶은 값
		for(int i = ptr-1 ; i>=0;i--) { //제일 위에 있는 데이터~ 0번데이터까지 1칸씩 내려오면서 탐색
			if(stk[i] == x) { //찾고자하는 데이터와 같은값을 발견했을 때
				return i; // 그 데이터가 있는 인덱스 번호 반환
			}
			
		}
		return -1; //찾고자 하는 값이 스택에 없는 경우에는 -1 반환
	}
	...
	public boolean isFull() {
		return ptr == capacity; //ptr이 capacity같으면(가득차있으면) -> true 반환
